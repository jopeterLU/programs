 function [I_DC,Mask,offset,MCC,PCC] = Func_DriftCorr(im,Mask,side)
...
% Cross-correlate and find maxima
r = xcorr2(rIm,fIm);
[max_cc, imax] = max(abs(r(:)));

% Find offset
[ypeak, xpeak] = ind2sub(size(r),imax(1));
offset = [(ypeak-size(HalfMask,1)) (xpeak-size(HalfMask,2))];
